...
class DataManager:
    ...
    def load_styles(self):
        ...
        if os.path.exists(self.styles_file):
            try:
                with open(self.styles_file, encoding="utf-8") as f:
    ...
    async def add_or_update_style(
        self, session_id: str, style_content: str, style_type: str
    ):
        """
        ...
        for style in self.styles[session_id]:
            if style["content"] == style_content and style["type"] == style_type:
                style["proficiency"] = min(
                    100, style.get("proficiency", 0) + 10
                )  # 熟练度增加，上限100
                style["last_updated"] = current_time
                self._dirty_styles = True
                await self._schedule_save()
