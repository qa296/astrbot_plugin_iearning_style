...
class StyleSelector:
    ...
    def select_styles_for_session(
        self, session_id: str, max_styles: int = 5, min_proficiency: int = 20
    ) -> dict[str, list[str]]:
        """
        ...
        language_styles = [
            ...
            and s.get("proficiency", 0) >= min_proficiency
        ]
        grammar_features = [
            s
            for s in styles
    ...
    def _weighted_random_selection(
        self, styles: list[dict[str, Any]], max_count: int
    ) -> list[str]:
        """
        根据熟练度加权随机选择风格。
